 ...
 * 所有订单页面
 */
public class OrderDetail_RV_Adapter extends BaseQuickAdapter<OrderInfoDetailBean> {
    ...
    protected void convert(BaseViewHolder helper, OrderInfoDetailBean item) {
        if(timelineView==null ) {
            timelineView = (TimeLineMarkerView) (helper.getView(R.id.tv_rv_activity_orderdetail_timelineview));
                ...
                helper.setTextColor(R.id.tv_rv_activity_orderdetail_time, context.getResources().getColor(R.color.line_5));
                helper.setTextColor(R.id.tv_rv_activity_orderdetail_content, context.getResources().getColor(R.color.line_5));
                timelineView.setBeginLine(context.getResources().getDrawable(R.color.blue_sky));
                timelineView.setEndLine(context.getResources().getDrawable(R.color.blue_sky));
                timelineView.setMarkerDrawable(context.getResources().getDrawable(R.drawable.timeline_bg_blue));

        if(helper.getAdapterPosition()==0 && "3".equals(state)){
